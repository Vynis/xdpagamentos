...
@Component({
  selector: 'ngx-conta-pagar-lista',
  templateUrl: './conta-pagar-lista.component.html',
  styleUrls: ['./conta-pagar-lista.component.scss']
})
export class ContaPagarListaComponent implements OnInit {
  ...
  columns = {
    ...
    fluxoCaixaFormatado: {
      title: 'Fluxo Caixa',
      type: 'custom',
      renderComponent: ContaPagarItensFluxoCaixaComponent,
  ...
  ngOnInit() {
    ...
    this.settings.actions.custom.push({ name: 'Baixar', title: '<i title="Baixar" class="nb-checkmark"></i>'});
  ...
  buscarDados(filtro: PaginationFilterModel) {
    this.contaPagarService.buscar(filtro).subscribe(
      res => {
        if (res.success) {
  ...
  onCustom(event) {
    switch (event.action) {
      case 'Baixar':
        this.route.navigateByUrl(`/pages/conta-pagar/baixa/${event.data.id}`);
        break;
      ...
      case AcoesPadrao.EDITAR:
        this.route.navigateByUrl(`/pages/conta-pagar/cadastro/edit/${event.data.id}`);
        break;
      case 'Restaurar':
        this.sweetAlertService.msgPadrao('Tem certeza que deseja restaurar este lançamento?', ' ', SweetAlertIcons.QUESTION).then(
          res => {
            if (res.isConfirmed) {
              this.fluxoCaixaService.restaurar(event.data.id,'CP').subscribe(
                res => {
                  if (res.success) 
