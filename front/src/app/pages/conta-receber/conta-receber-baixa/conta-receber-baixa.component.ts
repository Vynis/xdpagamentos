...
@Component({
  selector: 'ngx-conta-receber-baixa',
  templateUrl: './conta-receber-baixa.component.html',
  styleUrls: ['./conta-receber-baixa.component.scss']
})
export class ContaReceberBaixaComponent implements OnInit {
  formularioBaixa: FormGroup;
  contaReceber: ContaReceberModel;
  ...
  ngOnInit() {
    ...
    this.activatedRoute.params.subscribe(params => {
      const id = params.id;
      if (id && id > 0) {
        this.buscarDadosContasReceber(id);
      }
      else {
        this.voltar()
      }
  ...
  buscarDadosContasReceber(id: number) {
    this.contaReceberService.buscaPorId(id).subscribe(
      res => {
        ...
        if (this.contaReceber.valor == '' || this.contaReceber.valor == ' 0,00'){
  ...
  buscarItensFluxoCaixa(id) {
    this.fluxoCaixaService.buscarContas('CR', id).subscribe(
      res => {
        if (res.success) {
  ...
  prepararModel() : FluxoCaixaModel { 
    ...
    _model.valor = controls.tipoBaixa.value == 'N' ? formatarNumero(Number(this.valorResultado)) : formatarNumero(Number(controls.valor.value));
    _model.pcoId = controls.pcoId.value;
    _model.cocId = controls.cocId.value;
    _model.dtLancamento = controls.dtLancamento.value;
    _model.corId = this.contaReceber.id;
    _model.tipoPagamento = 'C';
  ...
  prepararModelAcDc() : FluxoCaixaModel { 
    ...
    _model.valor = this.calculoAcrescimo ? formatarNumero(controls.valorAcDc.value) : formatarNumero((-1) * Number(controls.valorAcDc.value));
    _model.pcoId = controls.pcoIdAcDc.value;
    _model.cocId = controls.cocId.value;
    _model.dtLancamento = controls.dtLancamento.value;
    _model.corId = this.contaReceber.id;
    _model.tipoPagamento = this.calculoAcrescimo ? 'C' : 'D';
  ...
  caculaSubTotal() {
    this.valorConta = formatarNumeroUS(this.contaReceber.valor);
