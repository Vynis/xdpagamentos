...
{
    public class VwGestaoPagamentoTransacoesMap : IEntityTypeConfiguration<VwGestaoPagamentoTransacoes>
    {
        public void Configure(EntityTypeBuilder<VwGestaoPagamentoTransacoes> builder)
        {
            builder.ToTable("VW_GESTAO_PAGAMENTO_TRANSACOES");
            ...
            builder.HasOne(c => c.Cliente).WithMany(c => c.ListaViewGestaoPagamentoTransacoes).HasForeignKey(c => c.CliId).IsRequired(false);
            builder.HasOne(c => c.FormaPagto).WithMany(c => c.ListaViewGestaoPagamentoTransacoes).HasForeignKey(c => c.FopId).IsRequired(false);
            builder.HasOne(c => c.RelContaEstabelecimento).WithMany(c => c.ListaViewGestaoPagamentoTransacoes).HasForeignKey(c => c.RceId).IsRequired(false);
